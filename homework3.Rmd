...
How many tables does the database have? ANS: it has 7 tables: appg_donations, appgs, member_appgs, members, parties, party_donations, and payments
...
```{r}
#set-up table in original database as database objects to be manipulated in R
payments_db <- dplyr::tbl(sky_westminster, "payments") 
members_db <- dplyr::tbl(sky_westminster, "members")
#this extracts an entity, need to use glimpse() to see the categories within
#from payments_db, extract member_id and value, and then sum value for each member_id
payments_db %>% 
  group_by(member_id) %>% 
  summarise(total = sum(value, na.rm = TRUE)) %>% #removed NAs while summing 
  #no need to use mutate, summarise is more efficient
  #left join using payments_db's category name (member_id)
...
ANS: Withers LLP, donated to Geoffrey Cox

```{r}
payments_db %>% 
  #add a "total" column for the sum of donations
  mutate(total = sum(value)) %>% 
  group_by(entity) %>% 
  #summarise - member id, and proportion of donation in total donations
  summarise(member_id, props = (sum(value, na.rm = TRUE))/total) %>% 
  left_join(members_db, by = c("member_id"="id")) %>% 
  arrange(desc(props)) %>% 
collect()
...
-   How many distinct entities who paid money to MPS are there? ANS: 2213 distinct entities

-   How many (as a number and %) donated to MPs belonging to a single party only?

```{r}

all_entities_count <- payments_db %>% 
  group_by(entity) %>% 
  #summarise member, total donations received, and number of distinct entities
  summarise(member_id, total = sum(value, na.rm = TRUE)) %>%
  filter(total >= 0) %>% 
  summarise(count = n()) %>% 
collect()

single_parties <- left_join(payments_db, members_db, by = c("member_id"="id")) %>% 
  #count no. of party_id per entity
  group_by(entity, party_id) %>%
  summarise(count = n()) %>% 
  #select entity again to filter out those donated to more than 1 party
  select(entity) %>% 
  #no. of entities that donated to 1 party only
  summarise(count = n()) %>%
  filter(count == 1 ) %>% 
  summarise(single_party = n()) %>% 
  collect() %>% 
  #proportion of entities donated to 1 party only
  summarise(prop_single = single_party/all_entities_count) %>% 
  collect()
...
#intuition: year, name of party (instead of party_id alone), total_year_donations (from "value", mp_id, and party_id), prop of total party donations received in year
...
#calculate the total yearly donations per party
total_donations <- party_donations_db %>% 
  mutate(year = year(date)) %>% 
  group_by(year, party_id) %>% 
  mutate(total_year_donations = sum(value, na.rm = TRUE)) %>% 
  ungroup() %>% 
  
  #calculate the yearly total of all party donations and finding the percentage each party receives
  group_by(year) %>%
  mutate(yearly_total = sum(value, na.rm = TRUE)) %>% 
  ungroup() %>% 
  mutate(prop = total_year_donations/yearly_total) %>% 
  
  #summarise into the required format
  left_join(parties_db, by = c("party_id"="id")) %>%
  summarise(year, name, total_year_donations, prop) %>%
  distinct(year, name, .keep_all=TRUE) %>% 
  collect()

ggplot(total_donations, aes(x = year, y = total_year_donations, fill = reorder(name,-total_year_donations))) +
  geom_bar(stat = "identity", position = "dodge") +
  theme_minimal() +
  labs(title = "Conservatives captured majority of donations",
